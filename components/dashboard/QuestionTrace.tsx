...
export default function QuestionTrace({ data, openChunkId }: { data: any; openChunkId?: string }) {
  ...
  // when openChunkId prop changes, open the corresponding type and chunk
  useEffect(() => {
    ...
    <div className="mx-auto max-w-4xl space-y-4">
      <div className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 text-sm text-slate-700">
          Evidence score: {data.evidence_score ?? 'N/A'}
        </div>
      </div>
        ...
        <div key={type} className="rounded-lg border border-slate-200 bg-white">
          ...
          >
            <span className="capitalize">
              {type} <span className="text-xs text-slate-500">({chunks.length})</span>
            </span>
          ...
          {openTypes[type] && (
            <div className="divide-y divide-slate-100 px-4 py-2">
              {chunks.map(chunk => (
                <div key={chunk.chunk_id} className="py-3">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <div className="text-sm font-medium text-slate-800">
                        {chunk.filename ?? 'source'}
                      </div>
                  ...
                  {openChunks[chunk.chunk_id] && (
                    <pre className="mt-3 whitespace-pre-wrap rounded-md border border-slate-100 bg-slate-50 p-3 text-sm text-slate-800">{`{${chunk.chunk_text}}`}</pre>
                  )}
                </div>
              ))}
            </div>
