    ...
    <div className="flex min-h-[calc(100vh-4rem)] flex-col md:min-h-screen">
      <div className="border-b border-slate-200 bg-white px-4 py-5 sm:px-6 lg:px-10">
        <div className="mx-auto max-w-4xl">
          ...
          <p className="mt-1 text-sm text-slate-500">Ask questions and review the evidence behind each answer.</p>
        </div>
      </div>
      ...
      <div className="flex-1">
        {messages.length === 0 ? (
          <div className="mx-auto flex min-h-[55vh] max-w-4xl items-center justify-center px-6 text-center">
            <div>
              <div className="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-2xl border border-blue-100 bg-blue-50 text-xl">✦</div>
              <h2 className="text-xl font-semibold text-slate-900">Ask a medical question</h2>
              <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">Answers are grounded in your uploaded documents and include evidence citations when available.</p>
            </div>
          </div>
        ) : (
          messages.map((m) => (m.role === 'user' ? <UserMessage key={m.id} text={m.text} /> : <ChatMessage key={m.id} response={m.response} />))
        )}
